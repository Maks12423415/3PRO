    ...
    const getData = async () => {
      try {
        const api = await fetch(
          `https://newsapi.org/v2/everything?q=bitcoin&apiKey=361b6ef9b64c4eab9af4bfcb11cd4243`
        );
          ...
          (article, index) =>
            article.author != null && (
              <Card
                key={index}
                className={
                  "flex flex-col items-center justify-center h-[650px] w-[500px]"
                }
              >
                <CardHeader>
                ...
                <CardContent className={"overflow-y-hidden flex-grow"}>
                  {article.content}
                </CardContent>
